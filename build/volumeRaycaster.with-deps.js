...
Core.prototype.setShaderName = function(value) {
    ...
    if(this._mode == "3d") {
      ...
      uNumberOfSlices: { type: "f", value:  (parseFloat(this.getSlicesRange()[1]) + 1.0)/1.000001 },
...
Core.prototype._initGeometry = function(geometryDimensions, volumeSizes) {
    ...
    this._geometry = geometryHelper.createBoxGeometry1(geometryDimensions, volumeSizes, this.zFactor);
    ...
    // console.log(-volumeSizes[0] / 2);
    // console.log(-volumeSizes[1] / 2);
    // console.log(-volumeSizes[2] / 2);
...
// Core.prototype.isVisible = function(x0,y0,z0,x,y,z,camx,camy,camz){
//   camVecX=camx-x0;
//   camVecY=camy-y0;
//   camVecZ=camz-z0;
//   // The face is visible if the angle between the normal to the face and the viewpoint is acute.
...
Core.prototype.calcXZ = function(xa,ya,za, xb,yb,zb ) {

  // ПОПРОБОВАТЬ ЗАФИКСИРОВАТЬ ax,bx,cx в geometryHelper и покрутить ничего больше не меняя
  tempxa=xa;
  tempya=ya;
  tempza=za;
  ...
  f = 15.0; //scaling можно использовать - или + от 11 до бесконечности
  xb = xa / f;
  yb = ya / f;
  zb = za / f;
  console.log("Xb = ",(xa).toFixed(2));
  console.log("Yb = ",(ya).toFixed(2));
  console.log("Zb = ",(za).toFixed(2));
  ...
  p = Math.pow(k,2.0)+1.0;
  // console.log("p= ",p);

  n = 2.0*zb*k-2.0*xb-2.0*k*k1;
  // console.log(n);

  m = Math.pow(xb,2.0)-2.0*zb*k1+Math.pow(zb,2.0)+Math.pow(k1,2.0)-Math.pow(r,2.0);
  // console.log(m);


  if (za <= zb){
    xf1 = (n*(-1.0) + Math.sqrt(Math.pow(n,2.0)-4.0*p*m))/(2.0*p);
    zf1 = k1-k*xf1;

    xf2 = (n*(-1.0) - Math.sqrt(Math.pow(n,2.0)-4.0*p*m))/(2.0*p);
    zf2 = k1-k*xf2;
  }
  else {
    xf1 = (n*(-1.0) - Math.sqrt(Math.pow(n,2.0)-4.0*p*m))/(2.0*p);
  ...
  lengthH=Math.sqrt(Math.pow(xn,2) + Math.pow(yn,2) + Math.pow(zn,2));
  xn2 = xn * (1 / lengthH) * k1 + k2;
  yn2 = yn * (1 / lengthH) * k1 + k2;
  zn2 = zn * (1 / lengthH) * k1 + k2;

  xn1 = (za * yf1 - ya * zf1) * (1 / lengthH) * k1 + k2; //*0.5
  yn1 = (xa * zf1 - za * xf1) * (1 / lengthH) * k1 + k2;
  zn1 = (ya * xf1 - xa * yf1) * (1 / lengthH) * k1 + k2;
  // console.log("XN= ",(xn1).toFixed(2));
  // console.log("YN= ",(yn1).toFixed(2));
  // console.log("ZN= ",(zn1).toFixed(2));
  ...
  // this._setGeometry1(this._geometry_dimensions, this.getVolumeSizeNormalized(),xf1,yf1,zf1,xf2,yf2,zf2,xn1,yn1,zn1,xn2,yn2,zn2,xa,ya,za);
  this._setGeometry1(this._geometry_dimensions, this.getVolumeSizeNormalized(),xf1,yf1,zf1,xf2,yf2,zf2,xn1,yn1,zn1,xn2,yn2,zn2,tempxa,tempya,tempza);

}

Core.prototype._setGeometry = function(geometryDimensions, volumeSizes) {
    // geometryDimensions["xmin"]=0.9;
    var geometryHelper = new VRC.GeometryHelper();
...
Core.prototype._setGeometry1 = function(geometryDimensions, volumeSizes, aX, aY, aZ, bX, bY, bZ, xn1, yn1, zn1, xn2, yn2, zn2, camX, camY, camZ) {
    // geometryDimensions["xmin"]=0.9;
    var geometryHelper = new VRC.GeometryHelper();
    var geometry      = geometryHelper.createBoxGeometry1(geometryDimensions, volumeSizes, 1.0, aX, aY, aZ, bX, bY, bZ, xn1, yn1, zn1, xn2, yn2, zn2, camX, camY, camZ);
    //var geometry      = geometryHelper.createBoxGeometry(geometryDimensions, volumeSizes, this.zFactor);
    var colorArray    = geometry.attributes.vertColor.array;
    var positionArray = geometry.attributes.position.array;
...
Core.prototype.setSlicesRange = function(from, to) {
    ...
    if(this._mode == "3d") {
        this._secondPassSetUniformValue("uNumberOfSlices", (parseFloat(this.getSlicesRange()[1]) + 1.0)/1.000001);
    }
};
...
Core.prototype.setSosThresholdBot = function(sosThresholdBot) {
    console.log("Core: setSosThresholdBot()");
    this._sosThresholdBot = sosThresholdBot;
    if(this._mode == "3d") {
        this._secondPassSetUniformValue("uSosThresholdBot", this._sosThresholdBot);
    }
};

Core.prototype.setSosThresholdTop = function(sosThresholdTop) {
    console.log("Core: setSosThresholdTop()");
    this._sosThresholdTop = sosThresholdTop;
    if(this._mode == "3d") {
        this._secondPassSetUniformValue("uSosThresholdTop", this._sosThresholdTop);
    }
};

Core.prototype.setAttenThresholdBot = function(attenThresholdBot) {
    console.log("Core: setAttenThresholdBot()");
    this._attenThresholdBot = attenThresholdBot;
    if(this._mode == "3d") {
        this._secondPassSetUniformValue("uAttenThresholdBot", this._attenThresholdBot);
    }
};

Core.prototype.setAttenThresholdTop = function(attenThresholdTop) {
    console.log("Core: setAttenThresholdTop()");
    this._attenThresholdTop = attenThresholdTop;
    if(this._mode == "3d") {
        this._secondPassSetUniformValue("uAttenThresholdTop", this._attenThresholdTop);
    }
};
...
// console.log(this._camera.position.y);
        console.log("");
        console.log("camX = ",(this._camera.position.x).toFixed(2));
        console.log("camY = ",(this._camera.position.y).toFixed(2));
        console.log("camZ = ",(this._camera.position.z).toFixed(2));


        this.calcXZ(this._camera.position.x,this._camera.position.y,this._camera.position.z,0,0,0);
        console.log("");
...
(function(namespace) {
    var VolumeRaycaster = function(config) {
        ...
        me.isVisible = function(x0,y0,z0,x,y,z,camx,camy,camz){
          me._core.isVisible(x0,y0,z0,x,y,z,camx,camy,camz);
          me._needRedraw = true;
          return me._core.isVisible();
        };


        me.calcXZ = function(xa,ya,za, xb,yb,zb){
          me._core.calcXZ(xa,ya,za, xb,yb,zb);
          me._needRedraw = true;
        };
        ...
        ////////////////////////////////////////////////////////////
        me.setSosThresholdBot = function(sosThresholdBot) {

            me._core.setSosThresholdBot(sosThresholdBot);
            me._needRedraw = true;
        };

        me.setSosThresholdTop = function(sosThresholdTop) {

            me._core.setSosThresholdTop(sosThresholdTop);
            me._needRedraw = true;
        };

        me.setAttenThresholdBot = function(attenThresholdBot) {

            me._core.setAttenThresholdBot(attenThresholdBot);
            me._needRedraw = true;
        };

        me.setAttenThresholdTop = function(attenThresholdTop) {

            me._core.setAttenThresholdTop(attenThresholdTop);
            me._needRedraw = true;
        };
        ////////////////////////////////////////////////////////////
       ...
       me.setMaxRefl = function(value) {
        ...
        me.setConfig = function(config, onLoadImage, onLoadImages) {
            ...
            if(config['render_canvas_size'] != undefined) {
              ...
              if(config['sosThresholdBot'] != undefined) {
                me._core.setSosThresholdBot( config['sosThresholdBot'] );
            }

            if(config['sosThresholdTop'] != undefined) {
                me._core.setSosThresholdTop( config['sosThresholdTop'] );
            }

            if(config['attenThresholdBot'] != undefined) {
                me._core.setAttenThresholdBot( config['attenThresholdBot'] );
            }

            if(config['attenThresholdTop'] != undefined) {
                me._core.setAttenThresholdTop( config['attenThresholdTop'] );
            }
            ///////////////////////////////////////////////////////////////////////////////////////////////////////
